...
if (isset($_GET['filter']) and $_GET['filter'] != 'tanggal') {
	...
	switch ($filter) {
		case 'semua':
			$q1 = "SELECT * FROM penjualan order by tgl_penjualan asc";
			...
			<div class="col-10">
				<table id="myTable" class="table">
					...
					foreach ($sql as $data) {
						...
						// if ($jb = mysqli_fetch_assoc($sql_jb)) {
						// 	$data['kredit'] = array('tanggal' => $jb['tanggal'], 'transaksi' => ($jb['kodeobat']) ? $jb['kodeobat'] : $jb['lainnya'], 'jenis' => $jb['jenis'], 'harga' => $jb['harga']);
						// }
						// print_r($data);
						// die;

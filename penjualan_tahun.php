...
if (!isset($_SESSION["login1"])) { // status login menggunakan session, jika login gagal maka kembali ke login.php
	...
	$QuerySql = "SELECT *,sum(harga_obat*jumlah_terjual) as total FROM `tabel_penjualan`, `obat` WHERE tabel_penjualan.kode_obat=obat.kode_obat GROUP BY  year(tanggal_terjual)";//fungsi untuk mengambil jumlah obat berdasarkan tahun
...
<table class="table is-fullwidth" >
  ...
  </thead> <!-- head tabel dari = tanggal, total penjualan -->
		<?php
			foreach ($SQL as $key) {
				echo "<tr>
						<td>$key[tanggal_terjual]</td>
						<td>$key[total]</td>
				</tr>"; //untuk menampilkan isi dari atribut di dalam tabel yaitu (tanggal terjual, total)

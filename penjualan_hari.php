...
if (!isset($_SESSION["login1"])) {//jika login gagal maka kembali login.php
	...
	$QuerySql = "SELECT *,sum(jumlah) as jumlah,sum(obat.harga) as harga_obat,sum(obat.harga)*sum(jumlah) as total FROM `jualbeli`, `obat` WHERE jualbeli.kodeobat=obat.kode_obat and jualbeli.jenis='debit' group by day(tanggal)";//fungsi untuk memanggil [total jumlah terjual , harga jual , harga obat berdasarkan bulan terjual

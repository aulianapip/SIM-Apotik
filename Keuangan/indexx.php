...
if (isset($_GET['filter']) and $_GET['filter'] != 'tanggal' ) {
 ...
 switch ($filter) {
 case 'semua':
 $q = "SELECT jualbeli.tanggal as tanggal, jualbeli.kodeobat as transaksi, jualbeli.lainnya as trans, jualbeli.jumlah as jumlah, jualbeli.harga as harga, transaksi.jenis, jualbeli.jenis as jeniss,obat.harga,obat.nama_obat as nama, jualbeli.jumlah*obat.harga as subtotal from jualbeli join transaksi on jualbeli.id = transaksi.id left join obat on obat.kode_obat = jualbeli.kodeobat order by jualbeli.tanggal asc";
 break;
 case 'hari':
 $q = "SELECT jualbeli.tanggal as tanggal, jualbeli.kodeobat as transaksi, jualbeli.lainnya as trans, jualbeli.jumlah as jumlah, jualbeli.harga as harga, transaksi.jenis, obat.harga,obat.nama_obat as nama, jualbeli.jumlah*obat.harga as subtotal from jualbeli join transaksi on jualbeli.id = transaksi.id left join obat on obat.kode_obat = jualbeli.kodeobat where jualbeli.tanggal = curdate() order by jualbeli.tanggal asc";
 break;
 case 'pekan':
 $q = "SELECT jualbeli.tanggal as tanggal, jualbeli.kodeobat as transaksi, jualbeli.lainnya as trans, jualbeli.jumlah as jumlah, jualbeli.harga as harga, transaksi.jenis, obat.harga,obat.nama_obat as nama, jualbeli.jumlah*obat.harga as subtotal from jualbeli join transaksi on jualbeli.id = transaksi.id left join obat on obat.kode_obat = jualbeli.kodeobat where WEEK(jualbeli.tanggal) = WEEK(curdate()) order by jualbeli.tanggal asc";
 break;
 case 'bulan':
 $q = "SELECT jualbeli.tanggal as tanggal, jualbeli.kodeobat as transaksi, jualbeli.lainnya as trans, jualbeli.jumlah as jumlah, jualbeli.harga as harga, transaksi.jenis, obat.harga,obat.nama_obat as nama, jualbeli.jumlah*obat.harga as subtotal from jualbeli join transaksi on jualbeli.id = transaksi.id left join obat on obat.kode_obat = jualbeli.kodeobat where month(jualbeli.tanggal) = month(curdate()) order by jualbeli.tanggal asc";
 break;
 case 'tahun':
 $q = "SELECT jualbeli.tanggal as tanggal, jualbeli.kodeobat as transaksi, jualbeli.lainnya as trans, jualbeli.jumlah as jumlah, jualbeli.harga as harga, transaksi.jenis, obat.harga,obat.nama_obat as nama, jualbeli.jumlah*obat.harga as subtotal from jualbeli join transaksi on jualbeli.id = transaksi.id left join obat on obat.kode_obat = jualbeli.kodeobat where year(jualbeli.tanggal) = year(curdate()) order by jualbeli.tanggal asc";
 break;
 }
 ...
 $q = "SELECT jualbeli.tanggal as tanggal, jualbeli.kodeobat as transaksi, jualbeli.lainnya as trans, jualbeli.jumlah as jumlah, jualbeli.harga as harga, transaksi.jenis, obat.nama_obat as nama from jualbeli join transaksi on jualbeli.id = transaksi.id left join obat on obat.kode_obat = jualbeli.kodeobat where jualbeli.tanggal between '$awal' and '$akhir' order by jualbeli.tanggal asc"; // query untuk menampilkan data dalam database pada tabel tabel_penjualan,obat,dan transaksi
 ...
 $q = "SELECT jualbeli.tanggal as tanggal, jualbeli.kodeobat as transaksi, jualbeli.lainnya as trans, jualbeli.jumlah as jumlah, jualbeli.harga as harga, transaksi.jenis, obat.nama_obat as nama from jualbeli join transaksi on jualbeli.id = transaksi.id left join obat on obat.kode_obat = jualbeli.kodeobat or jualbeli.lain = NULL where jualbeli.tanggal= curdate() order by jualbeli.tanggal asc";
 $sql = mysqli_query($connect, $q);
...
<body>
 ...
 <td>
 <?php
 if ($data['jeniss'] == "debit") //jika jenis debit
